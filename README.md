...
**约定大于配置**：我们相信好的默认值比灵活的配置更重要。YoClaw 大部分行为都有合理的默认值，让你开箱即用，而不是在配置文件中迷失。

**零依赖部署**：Go 语言编译为单一二进制文件，无需安装任何运行时环境。下载、运行、开始使用——就这么简单。

**专注用户体验**：特别是在群聊场景下，YoClaw 会自动：
- 识别发送者身份并显示姓名
- 保留最近的消息上下文
- 只在被 @ 时响应，不打扰正常群聊
...
使用 Docker 可以快速部署 YoClaw，所有数据都存储在 `~/.wangshu` 目录中。
...
docker run -d \
  --name wangshu \
  -v ~/.yoClaw:/root/.wangshu \
  ghcr.io/yockii/wangshu:latest
```

**说明：**
- `-v ~/.wangshu:/root/.yoClaw` - 将本地目录挂载到容器，所有数据（配置、工作空间、技能、会话等）都会持久化
...
4. 配置机器人权限：
   ...
   - 开启以下权限：
     ...
     - `im:chat.members:read`（读取群组成员，群聊时识别用户身份）注意：`im:message:group_at_msg`（获取群组@消息）不再需要，否则会重复接收
