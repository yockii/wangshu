...
#### 1. 飞书渠道配置
建议在配置好大模型后，直接让望舒自动创建飞书应用，无需手动配置。
<details>
<summary>飞书渠道手动配置</summary>
...
#### 2. 微信渠道配置
望舒支持通过 iLink 协议连接私人微信账号。
<details>
<summary>微信渠道手动配置</summary>
**配置示例：**
...
**配置说明：**
- `type`: 固定为 `wechat_ilink`
- `enabled`: 是否启用该渠道
- `agent`: 关联的 Agent 名称
- `cred_path`: 凭证存储路径（可选，默认为 `.wechatbot/{渠道名}_credentials.json`）
</details>

**使用流程：**
1. 在配置界面或配置文件中添加微信渠道配置
2. 启动程序后，如果未检测到登录凭证，会自动弹出二维码窗口
3. 使用微信扫描二维码并在手机上确认登录
4. 登录成功后，凭证会自动保存，下次启动无需重复扫码

**注意事项：**
- 如果在界面上开启微信渠道，保存后会自动要求扫码登录
- 首次使用需要扫码登录
- 登录凭证会保存在本地，请妥善保管
- 如需更换账号，删除凭证文件后重新启动即可
